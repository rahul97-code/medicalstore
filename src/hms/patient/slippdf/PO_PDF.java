...
public class PO_PDF {
	...
	public PO_PDF(String po_number, boolean printWithAmount) throws DocumentException, IOException {
		...
		PdfPTable table2 = printWithAmount ? WithAmountBill(cell) : WithoutAmountBill(cell);
...
//	    System.out.println(d1);  
	...
	private PdfPTable WithoutAmountBill(PdfPCell cell) {
		    // Define column widths for 5 columns
		    float[] table2Cell = { 0.07f, 0.6f, 0.1f, 0.1f, 0.1f }; // S.No, Description, Pack Size, No. of Packs, Qty in Units

		    // Create the table with the defined column widths
		    PdfPTable table2 = new PdfPTable(table2Cell);
		    table2.getDefaultCell().setBorder(0); // No border
		    ...
		    cell.setRowspan(2); // Span across two rows
		    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		    table2.addCell(cell);
		    ...
		    // Establish connection to the database and fetch the result set
		    PODBConnection podbConnection = new PODBConnection();
		    ResultSet resultSet = podbConnection.retrieveItems(po_number); // Assuming 'po_number' is the PO identifier
		    int counter = 0; // Initialize serial number counter
		    ...
		    try {
		        ...
		        while (resultSet.next()) {
		            ...
		            // Add Description of Goods column
		            cell = new PdfPCell(new Phrase(resultSet.getObject(1).toString(), font5)); // Assuming 1 is the column for Description
		            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		            table2.addCell(cell);

		            // Add Pack Size column
		            cell = new PdfPCell(new Phrase(resultSet.getObject(12).toString(), font5)); // Assuming 12 is the column for Pack Size
		            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		            table2.addCell(cell);

		            // Add No. of Packs column
		            cell = new PdfPCell(new Phrase(resultSet.getObject(13).toString(), font5)); // Assuming 13 is the column for No. of Packs
		            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		            table2.addCell(cell);

		            // Add Qty in Units column
		            cell = new PdfPCell(new Phrase(resultSet.getObject(3).toString(), font5)); // Assuming 3 is the column for Qty in Units
		            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		            table2.addCell(cell);
		        }
		    ...
		    return table2; // Return the table object with the populated rows

	}
	private PdfPTable WithAmountBill(PdfPCell cell) {
...
//				}
				...
				cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				table2.addCell(cell);
				cell = new PdfPCell(new Phrase(resultSet.getObject(13)==null?"":resultSet.getObject(13).toString(), font5));
				cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				table2.addCell(cell);
	...
	public void getSupplierDetail(String index) {
		...
		ResultSet resultSet = suppliersDBConnection.searchSupplierWithId(index);
		try {
			while (resultSet.next()) {
	...
	public void getInfo() {
		...
		try {
			while (resultSet.next()) {
				drug_no=resultSet.getObject(2).toString();
				gst_no=resultSet.getObject(3).toString();
