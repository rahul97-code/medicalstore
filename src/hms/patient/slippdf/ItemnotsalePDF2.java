...
public class ItemnotsalePDF2 {
    ...
    public static void main(String[] args) {
        try {
            generateReport("2021-01-23", "2021-08-25");
    ...
    private static void addHeader(Document document) throws DocumentException {
        Paragraph header = new Paragraph("Items Not Sold Report", FONT_HELVETICA_11_BOLD);
    ...
    private static void addItemsNotSoldTable(Document document, String fromDate, String toDate) throws DocumentException, SQLException {
        ...
        table.addCell(new PdfPCell(new Phrase("Item ID", FONT_HELVETICA_10_BOLD)));
        table.addCell(new PdfPCell(new Phrase("Item Name", FONT_HELVETICA_10_BOLD)));
        table.addCell(new PdfPCell(new Phrase("Total Stock", FONT_HELVETICA_10_BOLD)));
        table.addCell(new PdfPCell(new Phrase("Doctor Refrence", FONT_HELVETICA_10_BOLD)));
        table.addCell(new PdfPCell(new Phrase("Vendor Name", FONT_HELVETICA_10_BOLD)));
//        table.addCell(new PdfPCell(new Phrase("Item Date", FONT_HELVETICA_10_BOLD)));

        try  {
        	BillingDBConnection con= new BillingDBConnection();
            ResultSet resultSet = con.retrieveAllSummeryItemsNotSale(fromDate, toDate);
            while (resultSet.next()) {
                table.addCell(new PdfPCell(new Phrase(resultSet.getObject(1).toString(), FONT_HELVETICA_9)));
                table.addCell(new PdfPCell(new Phrase(resultSet.getObject(2).toString(), FONT_HELVETICA_9)));
                table.addCell(new PdfPCell(new Phrase(String.valueOf(resultSet.getObject(5).toString()), FONT_HELVETICA_9)));
